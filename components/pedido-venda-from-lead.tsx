...
export default function PedidoVendaFromLead({
  ...
  const handleAdicionarItemCarrinho = (
    ...
    const novoItem = {
      ...
      CODLOCALORIG: "700",
      CODVOL: unidade || produto.CODVOL || "UN", // Mantém a unidade selecionada
      ...
      CODVOL_PADRAO: produto.CODVOL || "UN", // Novo: salva a unidade padrão original do catálogo
  ...
  const carregarTabelasPrecos = async () => {
    try {
      ...
      // 3. Se o parceiro tiver CODTAB, buscar no IndexedDB as tabelas reais (NUTABs) vinculadas a esse CODTAB
      if (codTabParceiro) {
        // Regra: Sempre buscar do IndexedDB
        const allTabelas = await OfflineDataService.getTabelasPrecos()
        const tabelasParceiro = allTabelas.filter((t: any) => String(t.CODTAB) === String(codTabParceiro))
  ...
  const selecionarParceiro = async (parceiro: any) => {
    ...
    setPedido(prev => {
      ...
      const novoEstado = {
        ...prev,
        ...dadosParceiro,
        // Populando campos de endereço para visualização no cabeçalho
    ...
    if (preferenciaEmpresaAtiva) {
      ...
      // Reset para lista completa se pref. desativada
            ...
            <Card className="rounded-3xl border-[#F2F2F2] shadow-sm overflow-hidden">
              ...
              <CardContent className="p-4 md:p-5 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  ...
                  {/* Novos campos de endereço (Apenas visualização) */}
                  <div className="space-y-1 md:space-y-2">
                    ...
                    <Input
                      ...
                      className="text-xs md:text-sm h-8 md:h-10 bg-gray-100/50 cursor-not-allowed"
                    />
                  </div>
