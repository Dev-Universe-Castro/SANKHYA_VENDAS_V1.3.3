...
export class OfflineDataService {
   ...
   * Centraliza a lógica para garantir que tanto o login quanto a carga manual
   * atualizem a interface imediatamente.
   */
  static atualizarCachesSessionStorage(data: any) {
    if (typeof window === 'undefined') return;

    try {
      console.log('💾 [CACHE] Atualizando sessionStorages...');

      const cacheMap = [
        { key: 'cached_parceiros', data: data.parceiros?.data },
        { key: 'cached_produtos', data: data.produtos?.data },
        ...
        { key: 'cached_excecoesPrecos', data: data.excecoesPrecos?.data },
        { key: 'cached_excecoes_precos', data: data.excecoesPrecos?.data },
        { key: 'cached_regrasImpostos', data: data.regrasImpostos?.data },
        { 
      ...
      console.log('✅ [CACHE] SessionStorages atualizados com sucesso.');
    } catch (error) {
  ...
  static async sincronizarTudo(prefetchData: any, triggerName: string = 'desconhecido') {
    ...
    try {
      ...
      if (prefetchData.produtos?.data) {
        ...
        if (isDelta) {
          promises.push(
            db.produtos.bulkPut(prefetchData.produtos.data).then(() =>
      ...
      if (prefetchData.parceiros?.data) {
        ...
        if (isDelta) {
          promises.push(
            db.parceiros.bulkPut(parceirosProcessados).then(() =>
          ...
          promises.push(
            db.parceiros.clear().then(() =>
              db.parceiros.bulkAdd(parceirosProcessados)
            ).then(() =>
      ...
      if (prefetchData.excecoesPrecos?.data) {
        if (isDelta) {
          promises.push(
            db.precos.bulkPut(prefetchData.excecoesPrecos.data).then(() =>
      ...
      if (prefetchData.regrasIcms?.data) {
        if (isDelta) {
          promises.push(
            db.regrasIcmsParceiroEmpresa.bulkPut(prefetchData.regrasIcms.data).then(() =>
          ...
          promises.push(
            db.regrasIcmsParceiroEmpresa.clear().then(() =>
              db.regrasIcmsParceiroEmpresa.bulkAdd(prefetchData.regrasIcms.data)
            ).then(() =>
      ...
      if (prefetchData.complementoParc?.data) {
        if (isDelta) {
          promises.push(
            db.complementoParc.bulkPut(prefetchData.complementoParc.data).then(() =>
          ...
          promises.push(
            db.complementoParc.clear().then(() =>
              db.complementoParc.bulkAdd(prefetchData.complementoParc.data)
            ).then(() =>
  ...
  static async getEmpresasPorParceiro(codParc: number) {
    try {
      ...
      const codEmps = Array.from(new Set(regras.map(r => r.CODEMP)));
