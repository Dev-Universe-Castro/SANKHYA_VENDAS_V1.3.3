...
export async function POST(request: NextRequest) {
  try {
    ...
    const [
      ...
      prefetchTiposNegociacao(idEmpresa, lastSync),
      prefetchTiposOperacao(idEmpresa, lastSync),
      prefetchPedidos(idEmpresa, userId, lastSync),
      prefetchFinanceiro(idEmpresa, userId, lastSync),
      prefetchUsuarios(idEmpresa, lastSync),
      prefetchVendedores(idEmpresa, lastSync),
      prefetchTabelasPrecos(idEmpresa, lastSync),
      prefetchExcecoesPrecos(idEmpresa, lastSync),
      prefetchTiposPedido(idEmpresa, lastSync),
      prefetchTabelasPrecosConfig(idEmpresa, lastSync),
      prefetchVolumes(idEmpresa, lastSync),
      prefetchRegrasImpostos(idEmpresa, lastSync),
      prefetchAcessosUsuario(userId, lastSync),
      prefetchEquipes(idEmpresa, lastSync),
      prefetchBairros(idEmpresa, lastSync),
      prefetchCidades(idEmpresa, lastSync),
      prefetchEmpresas(idEmpresa, lastSync),
      prefetchEstados(idEmpresa, lastSync),
      prefetchGruposProdutos(idEmpresa, lastSync),
      prefetchMarcas(idEmpresa, lastSync),
      prefetchRegioes(idEmpresa, lastSync),
      prefetchPoliticasComerciais(idEmpresa, lastSync),
      prefetchCampanhas(idEmpresa, lastSync),
      prefetchCampanhaItens(idEmpresa, lastSync),
      prefetchRegrasIcmsParceiroEmpresa(idEmpresa, lastSync),
      prefetchComplementoParc(idEmpresa, lastSync)
    ])
...
async function prefetchProdutos(idEmpresa: number, lastSync: string | null): Promise<{ count: number, data: any[] }> {
  try {
    ...
    `
    const binds: any = { idEmpresa };
    // AS_PRODUTOS não possui DTALTER, então fazemos Full Sync por enquanto
    /*
    if (lastSync) {
...
async function prefetchVendedores(idEmpresa: number, lastSync: string | null): Promise<{ count: number, data: any[] }> {
  try {
    ...
    let sql = `
      SELECT CODVEND, APELIDO, ATIVO, DTALTER
      FROM AS_VENDEDORES
      WHERE ID_SISTEMA = :idEmpresa AND ATIVO = 'S'
    `
...
async function prefetchFinanceiro(idEmpresa: number, userId: number, lastSync: string | null): Promise<{ count: number, data: any[] }> {
  try {
    ...
    // AS_FINANCEIRO não possui DTALTER, então fazemos Full Sync por enquanto
    /*
    if (lastSync) {
      sql += ` AND (f.DTALTER > TO_TIMESTAMP(:lastSync, 'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') OR f.DTALTER IS NULL)`;
      binds.lastSync = lastSync;
    }
    */
